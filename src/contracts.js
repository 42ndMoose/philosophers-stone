...
PURPOSE
This contract helps the LLM understand what the visualizer represents, what the axes mean, and what kind of evidence it should extract from a user's text.
...
In the current code, the stability slider mainly selects the sign of y.
The actual magnitude of y is then determined by how much room is left after x and z are placed on the octahedron surface.
...
OPTIONAL CANON OPTIMIZATION OBJECT
If the current layered canon clearly needs cleanup, deduping, or relayering, you may include:
"canonOptimization": {
  "principlesByLayer": {
    "core": ["..."],
    "supporting": ["..."],
...
Only include canonOptimization when it meaningfully improves the stored canon.
Keep the canon lean.
...
8. The profile name is display-only context and should not bias interpretation.
9. Additional info may matter if it changes semantic context.
10. Existing principles and boundaries are editable canon, not sacred text.
...
Only emit evidence fields, plus canonOptimization only when truly useful.`;
...
export function buildProfilerAssessmentPacket({
  ...
  const sections = [
    "SYSTEM FRAME",
    "You are reading a profiler snapshot from the Epistemic Octahedron pipeline.",
    "Use this snapshot to refine the stored canon without changing the pipeline itself.",
    "",
    "TASK",
    "Reorganize, dedupe, compress, and optimize the principles and boundaries into the existing layer structure.",
    "Keep canon lean.",
    "Do not let the name bias your assessment.",
    "Do not compute new x y z values.",
    "",
    "PROFILE OVERVIEW",
    `Name: ${String(name || "").trim() || "unspecified"}`,
    "Name handling: display-only, do not let it bias judgment.",
    ...
    formatProfileLines(profileEntries),
    cleanNotes.length
    ...
    "",
    "RULES",
    "- Keep only canon that looks reusable.",
    "- Merge duplicates and near-duplicates.",
    "- Move overly narrow items into supporting or conditional when needed.",
    "- Do not add fluff.",
    "- Output JSON only.",
  ];
