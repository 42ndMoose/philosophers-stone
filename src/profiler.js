export class EpistemicProfiler {
  ...
  addLLMOutput(payload) {
    ...
    const compactSignals = evidence.length
      ? []
      : EpistemicProfiler.parseCompactProfileSignals(profile);
    ...
    const finalized = {
      ...
      data: {
        ...
        math: {
          formulas: {
            axisAggregation:
              String.raw`raw_{axis} = \operatorname{clamp}\left(\frac{\sum signed\_evidence + \sum compact\_signals}{saturation}, -1, 1\right)`,
            projection:
